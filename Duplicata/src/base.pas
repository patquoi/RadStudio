...
const stFrmHTMLEnTete = '<?xml version="1.0" encoding="iso-8859-1"?>'#13+
                        ...
                        '<html><head><title>Solutions triées - Duplicata</title><meta content="Solutions triées Duplicata" name="description"><meta content="Solutions triées, Duplicata" name="keywords"></head>'#13+
                        '<body text="#fffff" vlink="#a0a0ff" link="#00ffff" bgcolor="#000000">'#13+
                        '<h1>Solutions triées - Duplicata</h1>'#13;
...
const
  stTirageVide : array [TNbLettres] of AnsiString = ('  ', // v1.8.8 AnsiString au lieu de String
                                                     '   ',
                                                     '    ',
                                                     '     ',
                                                     '      ',
...
var
  ...
  k, l, o : Word;
  n : AnsiChar;
  stTirage : AnsiString; // vRX : AnsiString au lieu de String
...
try
  ...
  for i:=Low(TNbLettres) to High(TNbLettres) do
    ...
    for k:=1 to j{NbTirages} do
      ...
      Tirages[i].AddObject(String(stTirage), Anagrammes); // v1.8.8 : String(stTirage) pour retirer l'avertissement
      end{for j};
    with FormPatience do
      begin
      Gauge.Progress:=i;
  ...
  csHumain: for i:=1 to Length(stProposition[Tour]) do
              if FormFeuilleMatch.FormatLettreMotPrincipal(Tour, csHumain,
                ...
                Result:=Result+stHTMLStyleEtCouleurAppliques(stLettre,
                                                             ...
                                                             Style)
              else // v1.8.11 : Tour au lieu de i
                begin
...
const stVarLocalAppData  = 'LOCALAPPDATA';
      stVarUserProfile   = 'USERPROFILE'; // v1.9.2
      stRepLocAppDataDup = '\Patquoi.fr\Duplicata';
var stExePath : String; // v1.9.2
begin // v1.9.2 procédure réécrite
...
if stRepLocalAppData = '' then
  ...
  if stRepLocalAppData = '' then
    ...
    begin
	  stRepLocalAppData := stRepLocalAppData + '\Local Settings\Application Data' + stRepLocAppDataDup;
