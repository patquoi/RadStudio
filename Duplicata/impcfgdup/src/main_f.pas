...
const stFiltreDupIni      = 'Duplicata.ini';
//---------------------------------------------------------------------------
...
const stFiltreFMatch      = 'Duplicata.html';
      stFiltreParties     = 'Parties.html';
      stFiltreRecords     = 'Records.html';
...
var stLecteurCourant,
    stCheminCibleInstall  : String; // Transmis en paramètres via Insduplicata
...
if stChemin = '' then // Lancement depuis le bouton Rechercher (appel initial)
...
else // On a un chemin : on recherche d'abord Duplicata.ini
  begin
  ...
  if (slCheminsTrv.IndexOf(stChemin) = -1) then // Pas déjà trouvé
    if FindFirst(stChemin+stFiltreDupIni, faNormal, sr)=0 then
...
begin
AffecteImage(5, FichierExistantEtDeTailleNonNulle(slCheminsTrv[slCheminsTrv.Count-1]+stFiltreDupIni));
AffecteImage(6, FichierExistantEtDeTailleNonNulle(slCheminsTrv[slCheminsTrv.Count-1]+stFiltreFMatch));
AffecteImage(7, FichierExistantEtDeTailleNonNulle(slCheminsTrv[slCheminsTrv.Count-1]+stFiltreRecords));
AffecteImage(8, FichierExistantEtDeTailleNonNulle(slCheminsTrv[slCheminsTrv.Count-1]+stFiltreParties));
AffecteImage(9, FichierExistantEtDeTailleNonNulle(slCheminsTrv[slCheminsTrv.Count-1]+stFiltreDefis));
...
begin
if OpenDialogDupIni.Execute then
  begin
  LabeledEditDossierCourant.Text:=ExtractFilePath(OpenDialogDupIni.FileName);
...
//---------------------------------------------------------------------------
const stTitreMessage = 'Import de configuration Duplicata';
//---------------------------------------------------------------------------
...
case Timer.Tag of
 ...
 1: begin // 2. EXISTENCE de Duplicata.ini dans %LocalAppData% ?
    Tag:=1; // Busy
    if FichierExistantEtDeTailleNonNulle(LabeledEditLocalAppData.Text+'\Duplicata.ini') then
      begin
      AffecteImage(2, False);
      Timer.Enabled:=False;
      with Application do
        begin
        MessageBox('Vous avez déjà une configuration Duplicata dans votre dossier cible des données d''application.'#13#13+
 ...
 2: begin
    ...
    ButtonSelectionner.Enabled:=True;
    Timer.Enabled:=False;
    Application.MessageBox('Vous n''avez pas de configuration Duplicata dans votre dossier cible des données d''application.'#13#13+
                           'Si vous avez déjà installé Duplicata dans un autre dossier, vous pouvez importer la configuration en lançant une recherche ou en sélectionnant vous même le fichier de configuration.', stTitreMessage, MB_ICONINFORMATION);
