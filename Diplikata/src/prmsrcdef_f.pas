...
implementation
//---------------------------------------------------------------------------
uses System.IniFiles, Winapi.ShellApi, base, definition_f, main_f;
//---------------------------------------------------------------------------
{$R *.dfm}
//---------------------------------------------------------------------------
...
for i:=1 to NbSrcDef do
  if (Trim((FindComponent('Edit'+IntToStr(i)) as TEdit).Text)='') or
    ...
    begin
    FormMain.AfficheMessage('Les champs nom et adresses sont obligatoires !',
                            Format('Définition incomplète de la source n°%d', [i]),
                            MB_ICONHAND);
    Abort
    end;
...
if FormDefinitionMot.EditMot.Text<>'' then
  LabeledEditTest.Text:=FormDefinitionMot.EditMot.Text; // sinon 'Diplikata'
