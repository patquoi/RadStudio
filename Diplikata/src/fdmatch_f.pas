...
//---------------------------------------------------------------------------
type
  TLargeurFDM = (lLarge, lEtroite); // v1.6.5
  ...
  private
    FLargeur : TLargeurFDM;
  public
    property Largeur : TLargeurFDM read FLargeur;
    procedure InitialisePartie;
    function NbRetoursChariots(const Debut, Fin : Integer) : Integer; // v1.7.4
    ...
    function FormatLettreMotPrincipal(const Tour : Integer; const Cible : TCibleStats; const iLettre : Integer;
                                      ...
                                      var PoseJoker : Boolean; // vKA : Permet d'afficher en couleur et non plus en minuscule (réservé aux doubles lettres)
...
const
  ...
  LargeurClientFdMMin = 761; // v1.7.4
...
implementation
//---------------------------------------------------------------------------
uses System.Math, main_f, tirage_f;
//---------------------------------------------------------------------------
{$R *.dfm}
//---------------------------------------------------------------------------
const
...
//---------------------------------------------------------------------------
function TFormFeuilleMatch.NbRetoursChariots(const Debut, Fin : Integer) : Integer; // v1.7.4
var NvDebut : Integer;
begin
Result:=0;
...
var i,
    ...
    NbJetonsPlaces,
    Diminution,
    RetoursChariots, // v1.7.4
    Taille (* v1.7.4 *) : Integer;
    iChevalet           : TOrdreJetonChevalet;
    Style               : TFontStyles;
    stJokersSol,        // vKA : pour repérer les jokers à peindre en gris foncé
    stJokersPrp,        // vKA : pour repérer les jokers à peindre en gris foncé
    stTirage            : String;
...
iChevalet:=DebutPrp;
stJokersPrp:=''; // vKA : pour repérer les jokers à peindre en jaunegris foncé
for i:=0 to Length(stMotPrp)-1 do
  if p.c[XPrp+(i+iDebutMotPrp)*dx[dPrp],
    ...
    if (TypeJeton[ChevaletPrp[iChevalet]]=tjJoker) then // début vKA : pour repérer les jokers à peindre en gris foncé (on ne met plus en minuscule pour pas confondre avec les lettres doubles) 
      // stMotPrp[i+1]:=Chr(Ord(stMotPrp[i+1])+Ord('a')-Ord('A'));
      stJokersPrp:=stJokersPrp+'?'
    else
      stJokersPrp:=stJokersPrp+' '; // fin vKA : pour repérer les jokers à peindre en gris foncé
    Inc(iChevalet);
    end{if};
...
for i:=0 to Length(stMotSol)-1 do
  if p.c[XSol+(i+iDebutMotSol)*dx[dSol],
    ...
    if (TypeJeton[ChevaletSol[iChevalet]]=tjJoker) then // début vKA : pour repérer les jokers à peindre en gris foncé (on ne met plus en minuscule pour pas confondre avec les lettres doubles)
      //stMotSol[i+1]:=Chr(Ord(stMotSol[i+1])+Ord('a')-Ord('A'));
      stJokersSol:=stJokersSol+'?'
    else
      stJokersSol:=stJokersSol+' '; // fin vKA : pour repérer les jokers à peindre en gris foncé
    Inc(iChevalet);
    end{if};

Taille:=Length(RichEdit.Lines.Text); // v1.7.4
RetoursChariots:=NbRetoursChariots(0, Taille); // v1.7.4
iDebutDrnLigne:=Taille-RetoursChariots; // v1.5. v1.7.4 (RetoursChariots)
...
with RichEdit do
  ...
  SelLength:=Taille-iDebutDrnLigne; // v1.7.4 (Taille)
...
iDebutDrnLigne:=AnsiPos(Format(Copy(stFormatLigne, 1, 5), // v1.6.5 : Largeur. vKA : AnsiPos au lieu de Pos
                               [p.Tour]),
                        RichEdit.Lines.Text)-RetoursChariots; // v1.7.4 (RetoursChariots)
...
for i:=0 to Length(stMotPrp)-1 do
  ...
  if p.c[XPrp+(i+iDebutMotPrp)*dx[dPrp],
    ...
    Style:=[fsUnderline];
    if stJokersPrp[i+1]='?' then  // début vKA : on peint en gris foncé les jokers posés
      RichEdit.SelAttributes.Color:=CouleurEncreJokerPoseFdM
    else // fin vKA : on peint en gris foncé les jokers posés
...
for i:=0 to Length(stMotSol)-1 do
  ...
  if p.c[XSol+(i+iDebutMotSol)*dx[dSol],
    ...
    RichEdit.SelLength:=1;
    Style:=[fsUnderline];
    if stJokersSol[i+1]='?' then // début vKA : on peint en gris foncé les jokers posés
      RichEdit.SelAttributes.Color:=CouleurEncreJokerPoseFdM
    else // fin vKA : on peint en gris foncé les jokers posés
...
if iDebutLigne>0 then
  ...
  if not SubstitutJoker then // v1.5.6
    ...
    Bonus:=bAucun;
  PoseJoker:=(RichEdit.SelAttributes.Color=CouleurEncreJokerPoseFdM);
  Lettre:=RichEdit.SelText[1]; // v1.7.4 : SelText au lieu de Lines.Text
  Result:=True
  end
else
  Result:=False
...
begin // v1.6.5
Width:=FormTirage.Width;  // v1.7.4 même largeur que le tirage
end;
//---------------------------------------------------------------------------
