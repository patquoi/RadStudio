...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.ComCtrls, Vcl.Menus, base;
//---------------------------------------------------------------------------
...
type
  ...
  public
    ...
    procedure DefinitDimensionsParDefaut; // v1.7.4 : plus de paramètres FormTirageClient
...
try
  ...
  DessineTirage;
  // v1.7.4 : il faut utiliser Hide/Show pour déclencher FormShow ET faire apparaître la fenêtre
...
case Key of // v1.7.4 : Contrôles à l'intérieur du case
...
begin
if 1+(X-2) div TailleCase > NbMaxPlacesChevalet then // v1.7.4
  ChoixJeton:=NbMaxPlacesChevalet
else // v1.7.4 : X peut être négatif !
...
with Canvas do
  ...
  for i:=Low(TOrdreJetonChevalet) to High(TOrdreJetonChevalet) do
    if Chevalet[i]>0 then
      ...
      if EstDoubleLettre[ContenuJeton] then
        begin
        Font.Size:=TaillePoliceLD;
        stLD:=UpperCase(stAffichageJeton[ContenuJeton]);
...
//---------------------------------------------------------------------------
procedure TFormTirage.DefinitDimensionsParDefaut; // v1.7.4 : plus de paramètres FormTirageClient
begin
