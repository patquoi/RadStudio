...
uses
  ...
  Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.ImgList, Vcl.ActnList, Vcl.Menus, Vcl.ComCtrls, Vcl.ExtCtrls, base;
//---------------------------------------------------------------------------
type
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
  TFormMain = class(TForm)
    ...
    ActionAffichageFrancais: TAction;
    ActionAffichageKreyolAyisyen: TAction;
    ...
    MenuItemOptionsPlacerPropositionAuto: TMenuItem;
    ...
    MenuItemAffichageDefisReleves1M15L: TMenuItem;
    MenuItemAffichageRapportsPourcentage: TMenuItem;
    MenuItemAffichageRapportNoteSur20: TMenuItem;
    MenuItemInfosAide: TMenuItem;
    MenuItemInfosAPropos: TMenuItem;
    MenuItemInfosInternes: TMenuItem;
    MenuItemFrancais: TMenuItem;
    MenuItemKreyolAyisyen: TMenuItem;
    ...
    procedure ActionAffichageLangueExecute(Sender: TObject); // vKA
  ...
  private
    TaillePlateau,
    TaillePolice,
    TaillePoliceLD, // vKA : Pour lettres doubles
    TailleCase,
    ChoixColonne,
    ChoixLigne,
...
var
  FormMain: TFormMain;
  stTitrePlateau                : String = 'Diplikata'; // v1.1 (const -> var) Pour changer l'initiale selon s'il y a mélange ou non
//---------------------------------------------------------------------------
const
  ...
  CouleurEncreJokerPoseFdM      : TColor = $808080;  // vKA : pour ne pas confondre avec les doubles lettres, on ne met plus en minuscule les jokers posés mais on les met en jaune.
...
implementation
//---------------------------------------------------------------------------
uses System.Math, System.StrUtils, System.IniFiles, Winapi.ShellApi,
  cststr, tirage_f, listemots_f, saisietirage_f, definition_f, rechdico_f, fdmatch_f,
  statspartie_f, situationjetons_f, propos_f, prmpose_f, solutions_f, propose_f, langue;
//---------------------------------------------------------------------------
{$R *.dfm}
//---------------------------------------------------------------------------
type
...
const
  ...
  stEntreeAffichageL            : String = 'L'; // Langue (FR ou KA). v1.8 multilingue
...
//---------------------------------------------------------------------------
procedure TFormMain.ActionActionsListerMotsLettresCheresExecute(Sender: TObject);
begin // v1.6
...
//---------------------------------------------------------------------------
procedure TFormMain.ActionAffichageFeuilleMatchDetailleeExecute(Sender: TObject);
var stNomFichierFdMD : String;
begin
if stNomFichierPartie='' then
  if ActionOptionsGenererAutoFDM.Tag=0 then // v1.3.5
    stNomFichierFdMD:=stRepLocalAppData+'Diplikata.dip' // v1.7.4 : stRepLocalAppData remplace ExtractFilePath(ParamStr(0))
  else
    begin
...
//---------------------------------------------------------------------------
procedure TFormMain.ActionAffichageListeDefisReleves1M15LExecute(Sender: TObject);
begin // v1.5.3
d15l.GenereListeDefis(stRepLocalAppData+ // v1.7.4 : ExtractFilePath(Application.ExeName) remplacé par stRepLocalAppData
...
// v1.5.6 : ajout du temps de réflexion (Partie/Diplikata)
// v1.7.4 : affichage 32/64 bits dans les informations internes après la version de Diplikata
const stFrmInfoPartie = 'Partie'#13#13' · Nom : %s.'#13' · Numéro : %s.'#13' · Version du format de fichier de partie : %3X.'#13' · Type de partie : %s.'#13' · Temps de réflexion : %s.'#13#13+
                        'Diplikata'#13#13' · Version : %s (%d bits)'#13' · Prochain n° de partie : %d.'#13' · Version courante du format de fichier de partie : %X.'#13' · Type de partie : %s.'#13' · Temps de réflexion : %s.';
begin // v1.5.3
...
begin
stNomFichierJournal:=stRepLocalAppData+'diplikata.log'; // v1.7.4 : stRepLocalAppData remplace ExtractFilePath(ParamStr(0))
  ...
  if p.EstJouable(ChoixColonne, ChoixLigne,
    ...
    if p.MotsFormesValables(ChoixColonne, ChoixLigne,
      ...
      if not p.TiragePropose[p.Tour] then // v1.5.3 : pas de record si tirage proposé
        r.TesteRecord(srProposition, nvrTour, trMax, ntrTailleMot, p.Tour, p.TailleMaxMotForme, 'avec le mot '+stLettresDoubles(p.stMotTailleMax)); // vKA : on affiche les lettres doubles (ajout de stLettresDoubles)
  ...
  while not ChoixFait do
    case FormSolutions.ChoisitSolution(slSolutions) of // v1.4
                  ...
                  with FormSolutions.ListBox.Items.Objects[FormSolutions.ListBox.ItemIndex] as TSolution do
                    ...
                    if not p.TiragePropose[p.Tour] then // v1.5.3 : pas de record si tirage proposé
                      ...
                      if not DemoEnCours then
                        ...
                        r.TesteRecord(srTop, nvrTour, trMax, ntrTailleMot, p.Tour, p.TailleMaxMotForme, 'avec le mot '+stLettresDoubles(p.stMotTailleMax)); // vKA : on affiche les lettres doubles (ajout de stLettresDoubles) 
      ...
      tpManuelle: begin // v1.4
                  FreeAndNil(Solution);
                  if FormPropositionPose.PropositionPose(Solution, ScorePose, // v1.7.4 : plus de paramètres X, Y, d, Debut, Fin
                        ...
                        if not DemoEnCours then
                          ...
                          if p.ScorePartie[p.Tour]=p.ScoreTop[p.Tour] then
                            ...
                            r.TesteRecord(srTop, nvrTour, trMax, ntrTailleMot, // v1.5.1 : Record de taille de mot ajouté également ici
                                          p.Tour, p.TailleMaxMotForme, 'avec le mot '+stLettresDoubles(p.stMotTailleMax)); // vKA : on affiche les lettres doubles (ajout de stLettresDoubles)
...
DetermineVersion;
// EnregistreExtension; // v1.6.4 -> Fait dans l'installation de Diplikata (mode unique) permettant une compatibilité avec Windows Vista/Seven
stNomFichierPartie:='';
PartieModifiee:=False;
...
TailleOk     : Boolean;
begin
if WindowState = wsMinimized then // v1.7.4
  begin
  with FormTirage do
...
try
  ...
  with Canvas do
    Repeat
      ...
      Repeat
       if not EstDoubleLettre[i] then // vKA : On exclue les doubles lettres car elles sont traitées spécifiquement après (boucle Repeat suivante)
  ...
  // début vKA : On traite les doubles lettres ici
  TaillePolice:=2;
  with Canvas do
    Repeat
      Font.Size:=TaillePolice;
      i:=Succ(Low(TTypeJeton));
      Repeat
       if EstDoubleLettre[i] then
         TailleOk:=(Max(TextWidth(UpperCase(stAffichageJeton[i])), // vKA : on utilise stAffichageJeton qui contient les doubles lettres (affichage réel sur les jetons). stLettreJeton est toujours de 1 caractère
                        TextHeight(UpperCase(stAffichageJeton[i])))<=TailleLettre)
       else
         TailleOk:=true;
       i:=Succ(i);
      Until (i=High(TTypeJeton)) or not TailleOk;
      if TailleOk then
        begin
        Self.TaillePoliceLD:=TaillePolice;
        Inc(TaillePolice);
        end;
    Until not TailleOk;
   // fin vKA
  ...
  // v1.7.4 : on repositionne Tirage (et feuille de match)
  if (FormTirage<>nil) and FormTirage.Visible then
...
begin
OpenDialog.InitialDir:=stRepLocalAppData // v1.7.4 : ExtractFilePath(Application.ExeName) remplacé par stRepLocalAppData
end;
//---------------------------------------------------------------------------
procedure TFormMain.SaveDialogShow(Sender: TObject);
begin
SaveDialog.InitialDir:=stRepLocalAppData // v1.7.4 : ExtractFilePath(Application.ExeName) remplacé par stRepLocalAppData
end;
//---------------------------------------------------------------------------
...
with Canvas do
  ...
  if (p=Nil) or (p.c[x, y]=0) then
   ...
   // début vKA : il faut traiter les doubles lettres séparément
   if (EstDoubleLettre[TypeJeton[p.c[X,Y]]]) or
      ((TypeJeton[p.c[X,Y]]=tjJoker) and // Double lettre sur joker
        EstDoubleLettre[p.LettreJoker[p.c[X,Y]]]) then
     begin
     Font.Size:=TaillePoliceLD;
     stLD:=UpperCase(p.stLettreLD(x,y));
     LargeurTexte:=TextWidth(stLD);
     TextOut(x1+(TailleCase-LargeurTexte) div 2, // vKA : on ne retire LargeurScore car celui-ci est situé en dessous
             y1,
             stLD);
     Brush.Style:=Mode
     end
...
with Canvas do
 ...
 DessineCoordonnees; // v1.7.4 : on dessine d'abord les coordonnées pour éviter le débordement sur les cases
...
if MelangeEnCours then
  begin
  AfficheMessage('Aucune partie n''est en cours ou la partie est terminée.', 'Enregistrement impossible', MB_ICONEXCLAMATION);
  Result:=True;
  end
else
  Result:=False
...
try
  ...
  with IniFile do
    ...
    LangueAffichage:=TLangue(IniFile.ReadInteger(stSectionAffichage, stEntreeAffichageL, 0)); // v1.8 multilingue
...
try
  ...
  with IniFile do
    ...
    WriteInteger(stSectionAffichage, stEntreeAffichageL, Ord(LangueAffichage)); // v1.8 multilingue
...
try
  ...
  with IniFile do
    ...
    if FormTirage<>nil then
      with FormTirage do
        begin
        DefinitDimensionsParDefaut; // v1.7.4 : plus de paramètres FormTirageClient
...
try
  ...
  with IniFile do
    ...
    stNomFichier:=stRepLocalAppData+'Parties.html'; // v1.7.4 : stRepLocalAppData remplace ExtractFilePath(ParamStr(0))
...
try
  ...
  with IniFile do
    begin
    stNomFichier:=stRepLocalAppData+'Records.html'; // v1.7.4 : stRepLocalAppData remplace ExtractFilePath(ParamStr(0))
    AssignFile(f, stNomFichier);
    Rewrite(f);
...
//---------------------------------------------------------------------------
procedure TFormMain.ActionAffichageLangueExecute(Sender: TObject); // vKA
begin // v1.8 multilingue
LangueAffichage:=TLangue((Sender as TAction).Tag)
end;
//---------------------------------------------------------------------------
end.
