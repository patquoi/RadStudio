...
const stFiltreDupIni      = 'Diplikata.ini';
//---------------------------------------------------------------------------
...
const stFiltreFMatch      = 'Diplikata.html';
...
var stLecteurCourant,
    stCheminCibleInstall  : String; // Transmis en paramètres via Insdiplikata
...
else // On a un chemin : on recherche d'abord Diplikata.ini
  begin
...
case Timer.Tag of
 ...
 1: begin // 2. EXISTENCE de Diplikata.ini dans %LocalAppData% ?
    Tag:=1; // Busy
    if FichierExistantEtDeTailleNonNulle(LabeledEditLocalAppData.Text+'\Diplikata.ini') then
      begin
      AffecteImage(2, False);
      Timer.Enabled:=False;
      with Application do
        begin
        MessageBox('Vous avez déjà une configuration Diplikata dans votre dossier cible des données d''application.'#13#13+
 ...
 2: begin
    ...
    ButtonSelectionner.Enabled:=True;
    Timer.Enabled:=False;
    Application.MessageBox('Vous n''avez pas de configuration Diplikata dans votre dossier cible des données d''application.'#13#13+
                           'Si vous avez déjà installé Diplikata dans un autre dossier, vous pouvez importer la configuration en lançant une recherche ou en sélectionnant vous même le fichier de configuration.', stTitreMessage, MB_ICONINFORMATION);
