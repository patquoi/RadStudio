...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.ComCtrls, base;
//---------------------------------------------------------------------------
type
  ...
  if ModeRecherche=mrMot then
    ...
    repeat
      ...
      if IndexMot>-1 then
        ...
        if slDelta8.IndexOf(stMot)>-1 then Memo.Lines.Add('*'+stMot) // v1.8
...
// v1.8 : Nouveautés ODS7 // v1.8 // v1.9 (ODS8)
slDelta8:=TStringList.Create;
slDelta8.Sorted:=true;
slDelta8.LoadFromFile(ExtractFilePath(ParamStr(0))+'delta8.txt');
end;
//---------------------------------------------------------------------------
...
if iTirageRecherche>-1 then
  ...
  for i:=0 to Anagrammes.NbAnagrammes-1 do
    ...
    if slDelta8.IndexOf(stAnagramme)>-1 then Memo.Lines.Add('*'+stAnagramme) // v1.8
