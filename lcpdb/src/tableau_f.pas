...
uses
  ...
  Vcl.Dialogs, Menus, ImgList, ExtCtrls, ComCtrls, ToolWin, System.ImageList,
...
//-----------------------------------------------------------------------------
  TFormTableau = class(TForm)
    ...
    ToolButtonDifficulte: TToolButton;
    ImageCollection: TImageCollection;
    VirtualImageListChiffresOperateurs: TVirtualImageList;
    VirtualImageListPlaques: TVirtualImageList;
    VirtualImageListDifficulte: TVirtualImageList; // v1.1
  ...
  private
    ...
    // Deb v1.3 : Support HDPI
    nPPI,
    HauteurDgtHDPI,
    LargeurDgtHDPI,
    LargeurEspHDPI,
    EspChfOperHDPI,
    HtrChfOperHDPI : Integer;
    // Fin v1.3 : Support HDPI
...
l:=3*LargeurDgtHDPI+6*EspChfOperHDPI; // v1.3 : Support HDPI
y1:=(64*nPPI) div 96; // v1.3 : Support HDPI
y2:=(144*nPPI) div 96; // v1.3 : Support HDPI
...
c:=IA.Compte div 100;
ddx:=(VirtualImageListChiffresOperateurs.Width-LargeurDgtHDPI) div 2;  // v1.3 : Support HDPI
ddy:=(VirtualImageListChiffresOperateurs.Height-HauteurDgtHDPI) div 2; // v1.3 : Support HDPI
y:=(y1-HauteurDgtHDPI) div 2-ddy; // v1.3 : Support HDPI
xd:=(ClientWidth-LargeurDgtHDPI) div 2-ddx; // v1.3 : Support HDPI
VirtualImageListChiffresOperateurs.Draw(Canvas, xd-LargeurDgtHDPI-EspChfOperHDPI, y, c); // v1.3 : Support HDPI
VirtualImageListChiffresOperateurs.Draw(Canvas, xd, y, d);
VirtualImageListChiffresOperateurs.Draw(Canvas, xd+LargeurDgtHDPI+EspChfOperHDPI, y, u); // v1.3 : Support HDPI

// 2. Affichage des plaques
for i:=Low(TPlaque) to High(TPlaque) do
  begin
  xp:=i*((ClientWidth-NbMaxPlaques*VirtualImageListPlaques.Width) div (1+NbMaxPlaques)+VirtualImageListPlaques.Width);
  VirtualImageListPlaques.Draw( Canvas,
                         xp-VirtualImageListPlaques.Width,
                         y1+(y2-y1-VirtualImageListPlaques.Height) div 2,
...
NbResultats:=0;
ChercherNombre:=True;
x:=EspChfOperHDPI; y:=(180*nPPI) div 96; // v1.3 : Support HDPI; // v1.3 : Support HDPI
for i:=2 to Length(stSolution) do
  if stSolution[i]<>' ' then
    begin
    iCar:=IndexChiffreOperateurASCII[Ord(stSolution[i])];
    VirtualImageListChiffresOperateurs.Draw(Canvas, x-(VirtualImageListChiffresOperateurs.Width-(LargeurChiffreOperateur[iCar]*nPPI) div 96) div 2, y, iCar+10); // v1.3 : Support HDPI
    Inc(x, (LargeurChiffreOperateur[iCar]*nPPI) div 96); // v1.3 : Support HDPI
    ...
    x:=EspChfOperHDPI; // v1.3 : Support HDPI
    Inc(y, HtrChfOperHDPI); // v1.3 : Support HDPI
    stNombreForme:='';
    ChercherNombre:=true;
    end;
end;
...
for i:=Low(TPlaque) to High(TPlaque) do
  if not PlaqueChoisie[i] and (stNombreForme=stPlaque[i]) then
    ...
    VirtualImageListChiffresOperateurs.Draw( Canvas,
                                      i*((ClientWidth-NbMaxPlaques*VirtualImageListPlaques.Width) div (1+NbMaxPlaques)+VirtualImageListPlaques.Width)-VirtualImageListPlaques.Width div 2-14,
                                      (128*nPPI) div 96, // v1.3 : Support HDPI
...
// Deb v1.3 : Support HDPI
nPPI:=Screen.PixelsPerInch;
HauteurDgtHDPI:=(HauteurDigit*nPPI) div 96;
LargeurDgtHDPI:=(LargeurDigit*nPPI) div 96;
LargeurEspHDPI:=(LargeurEspace*nPPI) div 96;
EspChfOperHDPI:=(EspaceChiffreOperateur*nPPI) div 96;
HtrChfOperHDPI:=(HauteurChiffreOperateur*nPPI) div 96;
// Fin v1.3 : Support HDPI
